...
def score_submission(submission_id, notify=True):
    ...
    try:
        with submission.task.data_file.open() as truth_file, submission.test_prediction_file.open() as prediction_file:
            results = compute_metrics(truth_file, prediction_file)
        score = submission.score_set.create(score=results)
        score.score = results
        ...
        submission.score_set.create(fail_reason=e.args[0])
        submission.save()
        if notify:
            send_mail(
        ...
        submission.save()
        submission.score_set.create(fail_reason="internal error")
        if notify:
            send_mail(
